		...
		<table class="table table-slim" id="stock_history_table">
			...
			@forelse($stock_history as $history)
				<tr>
					...
					<td>
						@if(!empty($history['production_job_id']))
							<a href="{{ route('production.show', $history['production_job_id']) }}" class="tw-font-semibold tw-text-indigo-600">
								{{ $history['job_number'] ?? 'Job' }}
							</a>
							@if(!empty($history['job_title']))
								<br><span>{{ $history['job_title'] }}</span>
							@endif
							@if(!empty($history['job_customer']))
								<br><small class="text-muted">{{ $history['job_customer'] }}</small>
							@endif
						@else
							--
						@endif
