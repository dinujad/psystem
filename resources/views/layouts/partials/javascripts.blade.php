...
<script type="text/javascript">
    $(document).ready(function() {
        ...
        // side bar toggle
        function getChiled($link) {
            var uid = $link.data('dd');
            if (uid) {
                var $byUid = $('[data-for="' + uid + '"]');
                if ($byUid.length) return $byUid;
            }
            var $n = $link.next(".chiled");
            if ($n.length) return $n;
            return $link.parent().children(".chiled").first();
        }

        function updateArrows() {
            $(".drop_down").each(function() {
                var $c = getChiled($(this));
                var $svg = $(this).find(".svg");
                if ($c.length && $c.is(":visible")) {
                    $svg.html('<path stroke="none" d="M0 0h24v24H0z" fill="none" /><path d="M6 9l6 6l6 -6" />');
                } else {
                    $svg.html('<path stroke="none" d="M0 0h24v24H0z" fill="none"/><path d="M15 6l-6 6l6 6" />');
                }
            });
        }
        ...
        $(".drop_down").click(function(event) {
            ...
            var $chiled = getChiled($(this));
            $(".chiled").not($chiled).slideUp(200, updateArrows);
            $chiled.slideToggle(200, updateArrows);
        });
