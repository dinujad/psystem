...
<div class="pos-tab-content">
    <div class="row">
        <div class="col-xs-3">
            <div class="form-group">
                ...
                {!! Form::select('sms_settings[sms_service]', ['nexmo' => 'Nexmo', 'twilio' => 'Twilio', 'textlk' => 'TextLK', 'other' => __('lang_v1.other')], $sms_service , ['class' => 'form-control', 'id' => 'sms_service']); !!}
            </div>
        </div>
    </div>
    ...
    <div class="row sms_service_settings @if($sms_service != 'textlk') hide @endif" data-service="textlk">
        <div class="col-xs-4">
            <div class="form-group">
                {!! Form::label('textlk_api_key', 'TextLK API Key:') !!}
                {!! Form::text('sms_settings[textlk_api_key]', !empty($sms_settings['textlk_api_key']) ? $sms_settings['textlk_api_key'] : env('TEXTLK_API_KEY'), ['class' => 'form-control','placeholder' => 'TextLK API Key', 'id' => 'textlk_api_key']); !!}
            </div>
        </div>
        <div class="col-xs-4">
            <div class="form-group">
                {!! Form::label('textlk_sender_id', 'TextLK Sender ID:') !!}
                {!! Form::text('sms_settings[textlk_sender_id]', !empty($sms_settings['textlk_sender_id']) ? $sms_settings['textlk_sender_id'] : env('TEXTLK_SENDER_ID'), ['class' => 'form-control','placeholder' => 'Sender ID', 'id' => 'textlk_sender_id']); !!}
            </div>
        </div>
        <div class="col-xs-4">
            <div class="form-group">
                {!! Form::label('textlk_url', 'TextLK URL:') !!}
                {!! Form::text('sms_settings[textlk_url]', !empty($sms_settings['textlk_url']) ? $sms_settings['textlk_url'] : env('TEXTLK_URL', 'https://app.text.lk/api/v3/sms/send'), ['class' => 'form-control','placeholder' => 'https://app.text.lk/api/v3/sms/send', 'id' => 'textlk_url']); !!}
            </div>
        </div>
    </div>
    <div class="row">
        <div class="col-xs-12">
            <hr>
            <h4 style="margin-top: 0;">WhatsApp Alerts (Linked Device)</h4>
            <p class="help-block">
                System WhatsApp messages (quotations, low stock, production) send through your
                <strong>linked WhatsApp device</strong>.
                <a href="{{ action([\App\Http\Controllers\WhatsappController::class, 'showQr']) }}" target="_blank">Link device (QR)</a>
                &nbsp;|&nbsp;
                <a href="{{ route('admin.whatsapp.emwa.index') }}" target="_blank">E MEDIA WhatsApp API</a>
            </p>
        </div>
        <div class="col-xs-4">
            <div class="form-group">
                {!! Form::label('admin_whatsapp_number', 'Admin WhatsApp Number:') !!}
                {!! Form::text('sms_settings[admin_whatsapp_number]', $sms_settings['admin_whatsapp_number'] ?? null, ['class' => 'form-control', 'placeholder' => '947XXXXXXXX', 'id' => 'admin_whatsapp_number']); !!}
                <p class="help-block">Low stock alert messages will be sent to this number via linked WhatsApp.</p>
            </div>
        </div>
    </div>
