    ...
    $('button.pos-express-finalize').click(function() {
        ...
        if (pay_method == 'credit_sale') {
            if (isWalkInCustomerSelected()) {
                toastr.error(LANG.credit_sale_not_allowed_for_walk_in_customer);
                return false;
            }
...
function isWalkInCustomerSelected() {
    ...
    return default_customer_id && customer_id == default_customer_id;
}

function toggleWalkInCreditSaleButton() {
    var credit_sale_btns = $('button.pos-express-finalize[data-pay_method="credit_sale"]');

    if (isWalkInCustomerSelected()) {
        credit_sale_btns.addClass('hide').prop('disabled', true);
        if ($('#is_credit_sale').length) {
            $('#is_credit_sale').val(0);
        }
    } else {
        credit_sale_btns.removeClass('hide').prop('disabled', false);
    }
}
