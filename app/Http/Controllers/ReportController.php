        ...
        if ($request->ajax()) {
            ...
            return Datatables::of($contacts)
                ...
                ->addColumn(
                    ...
                    $pay_url = action([\App\Http\Controllers\TransactionPaymentController::class, 'getPayContactDue'], [$row->id]) . '?type=sell';
                    $view_url = action([\App\Http\Controllers\SellController::class, 'index']) . '?customer_id=' . $row->id;
                    $ledger_url = action([\App\Http\Controllers\ContactController::class, 'show'], [$row->id]);

                    return '<div class="naya-report-actions" style="display:flex;flex-direction:column;gap:4px;min-width:120px;white-space:nowrap;">
                        <a href="' . $pay_url . '" class="pay_sale_due tw-dw-btn tw-dw-btn-xs tw-dw-btn-success tw-text-white">
                            <i class="fas fa-money-bill-alt"></i> ණය ගෙවීම
                        </a>
                        <a href="' . $view_url . '" target="_blank" class="tw-dw-btn tw-dw-btn-xs tw-dw-btn-info tw-text-white">
                            <i class="fas fa-list"></i> විකිණීම්
                        </a>
                        <a href="' . $ledger_url . '" target="_blank" class="tw-dw-btn tw-dw-btn-xs tw-dw-btn-outline">
                ...
                ->rawColumns(['total_purchase', 'total_invoice', 'due', 'name', 'total_purchase_return', 'total_sell_return', 'opening_balance_due', 'action'])
                ->make(true);
        }
