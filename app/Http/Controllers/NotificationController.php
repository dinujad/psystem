...
class NotificationController extends Controller
{
    ...
    public function send(Request $request)
    {
        ...
        try {
            ...
            $notification_type = (array) $request->input('notification_type', []);

            $emails_array = [];
            if (! empty($data['to_email'])) {
                $emails_array = array_filter(array_map('trim', explode(',', (string) $data['to_email'])));
            }
            ...
            if (array_key_exists($request->input('template_for'), $customer_notifications)) {
                if (in_array('email', $notification_type) && ! empty($emails_array)) {
                    try {
                        ...
                        \Log::emergency('Email send failed. File:'.$e->getFile().' Line:'.$e->getLine().' Message:'.$e->getMessage());
                        $channel_errors[] = 'Email failed: '.$e->getMessage();
                    }
                }
                if (in_array('sms', $notification_type)) {
                    try {
                        ...
                        \Log::emergency('SMS send failed. File:'.$e->getFile().' Line:'.$e->getLine().' Message:'.$e->getMessage());
                        $channel_errors[] = 'SMS failed: '.$e->getMessage();
                    }
                }
                if (in_array('whatsapp', $notification_type)) {
                    try {
                        ...
                        \Log::emergency('Whatsapp link failed. File:'.$e->getFile().' Line:'.$e->getLine().' Message:'.$e->getMessage());
                        $channel_errors[] = 'Whatsapp failed: '.$e->getMessage();
                    }
                }
            ...
            if ($sent_any) {
                $msg = __('lang_v1.notification_sent_successfully');
                if (! empty($channel_errors)) {
                    $msg .= ' ('.implode(' | ', $channel_errors).')';
                }
                $output = ['success' => 1, 'msg' => $msg];
            } else {
                $output = ['success' => 0, 'msg' => ! empty($channel_errors) ? implode(' | ', $channel_errors) : __('messages.something_went_wrong')];
            }
