...
class HomeController extends Controller
{
    ...
    public function index()
    {
        ...
        if (ProductionStageEmployee::isSectionOnlyUser($user)) {
            $sectionUrl = ProductionStageEmployee::dashboardUrl($user->id);
            if ($sectionUrl) {
        ...
        if ($this->canViewProductionDashboard()) {
            $showProduction = true;
            $productionStages = ProductionJob::allStages();
            $activeStages = array_keys(array_filter(
                $productionStages,
                fn ($label, $key) => $key !== 'completed',
            ...
            if (! auth()->user()->can('send_notifications') && ! auth()->user()->can('production.access')) {
                $allowed = ProductionStageEmployee::stagesForUser(auth()->id());
                $jobsQuery->whereIn('current_stage', $allowed ?: ['__none__']);
            }

            $activeJobs = $jobsQuery->limit(12)->get();

            foreach ($activeStages as $stageKey) {
                $isFullAccess = auth()->user()->can('send_notifications') || auth()->user()->can('production.access');
                if (! $isFullAccess) {
                    $allowed = ProductionStageEmployee::stagesForUser(auth()->id());
                ...
                $stageCounts[$stageKey] = ProductionJob::where('current_stage', $stageKey)->count();
            }
        }
    ...
    private function canViewProductionDashboard(): bool
    {
        ...
        return ProductionStageEmployee::where('user_id', $user->id)->exists();
    }
