...
class AdminlteCustomPresenter extends Presenter
{
    ...
    {
        $activeClass = (string) $this->getActiveStateOnChild($item);
        $uid = 'dd-' . md5($item->title);

        $dropdownToggle = '<a href="#" data-dd="' . $uid . '" title="" class="drop_down tw-flex tw-items-center tw-gap-3 tw-px-3 tw-py-2 tw-text-sm tw-font-medium tw-tracking-tight tw-text-gray-600 tw-transition-all tw-duration-200 tw-rounded-lg tw-whitespace-nowrap hover:tw-text-gray-900 hover:tw-bg-gray-100 focus:tw-text-gray-900 focus:tw-bg-gray-100 ' . $activeClass . '">' .
            $this->formatIcon($item->icon) . ' <span class="tw-truncate">' . $item->title . '</span>' .
        ...
        $childItems = $this->getChildMenuItems($item, $uid);

        return '<div class="' . $activeClass . '">' . $dropdownToggle . $childItems . '</div>' . PHP_EOL;
    }
     ...
     * @return string
     */
    public function getChildMenuItems($item, $uid = '')
    {
        ...
        $ddAttr = $uid ? ' data-for="' . $uid . '"' : '';

        $children = '<div class="chiled tw-relative tw-mt-2 tw-mb-4 tw-pl-11"' . $ddAttr . ' style="display:' . $displayStyle . '">'
        ...
        foreach ($item->getChilds() as $child) {
            ...
            $children .= '<a href="' . $child->getUrl() . '" class="tw-flex tw-text-sm tw-font-medium tw-tracking-tight tw-text-gray-600 tw-truncate tw-transition-all tw-duration-200 hover:tw-text-gray-900 tw-whitespace-nowrap' . $isActive . '">'
