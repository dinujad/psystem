...
class AdminSidebarMenu
{
    ...
    public function handle($request, Closure $next)
    {
        ...
        Menu::create('admin-sidebar-menu', function ($menu) {
            ...
            // `enabled_modules` should be an array, but depending on how the session is built
            // it may come as JSON string / collection / null, which can break `in_array()` (PHP 8+).
            if ($enabled_modules instanceof \Illuminate\Support\Collection) {
                $enabled_modules = $enabled_modules->all();
            }

            if (is_string($enabled_modules)) {
                $val = $enabled_modules;
                // Decode up to 3 times to handle double-encoded JSON strings.
                for ($i = 0; $i < 3; $i++) {
            ...
            if (!is_array($enabled_modules) && !is_string($enabled_modules)) {
                $enabled_modules = [];
            }
            ...
            if (auth()->user()->can('send_notifications')) {
                $menu->dropdown('WhatsApp', function ($sub) {
                    ...
                    $sub->url(action([\App\Http\Controllers\WhatsappFlowController::class, 'index']), 'Bot Flows', ['icon' => '', 'active' => request()->segment(1) == 'admin' && request()->segment(2) == 'whatsapp' && in_array(request()->segment(3), ['flows', 'steps'])]);
                    $sub->url(action([\App\Http\Controllers\WhatsappBotController::class, 'index']), 'Bot Conversations', ['icon' => '', 'active' => request()->segment(1) == 'admin' && request()->segment(2) == 'whatsapp' && request()->segment(3) == 'conversations']);
                    $sub->url(action([\App\Http\Controllers\WhatsappLabelController::class, 'index']), 'Labels', ['icon' => '', 'active' => request()->segment(1) == 'admin' && request()->segment(2) == 'whatsapp' && request()->segment(3) == 'labels']);
                    $sub->url(action([\App\Http\Controllers\WhatsappAgentController::class, 'agents']), 'Agents', ['icon' => '', 'active' => request()->segment(1) == 'admin' && request()->segment(2) == 'whatsapp' && request()->segment(3) == 'agents']);
                    $sub->url(route('admin.whatsapp.reports'), 'Inquiry Reports', ['icon' => '', 'active' => request()->segment(1) == 'admin' && request()->segment(2) == 'whatsapp' && request()->segment(3) == 'reports']);
                    $sub->url(route('admin.whatsapp.emwa.index'), 'E MEDIA WhatsApp API', ['icon' => '', 'active' => request()->segment(1) == 'admin' && request()->segment(2) == 'whatsapp' && request()->segment(3) == 'emwa-api']);
                }, ['icon' => '<svg aria-hidden="true" class="tw-size-5 tw-shrink-0" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" fill="none" stroke-linecap="round" stroke-linejoin="round">
                    <path stroke="none" d="M0 0h24v24H0z" fill="none"></path>
                    <path d="M3 21l1.65 -3.8a9 9 0 1 1 3.4 2.9l-5.05 .9"></path>
                    <path d="M9 10a.5 .5 0 0 0 1 0v-1a.5 .5 0 0 0 -1 0v1a5 5 0 0 0 5 5h1a.5 .5 0 0 0 0 -1h-1a.5 .5 0 0 0 0 1"></path>
                  </svg>', 'active' => request()->segment(1) == 'whatsapp' || (request()->segment(1) == 'admin' && request()->segment(2) == 'whatsapp')])->order(16);
            } elseif (auth()->user()->can('whatsapp.agent')) {
                $menu->dropdown('WhatsApp', function ($sub) {
                    $sub->url(action([\App\Http\Controllers\WhatsappController::class, 'inbox']), 'Inbox', ['icon' => '', 'active' => request()->segment(1) == 'whatsapp' && request()->segment(2) == 'inbox']);
                    $sub->url(route('admin.whatsapp.reports'), 'My Inquiries', ['icon' => '', 'active' => request()->segment(1) == 'admin' && request()->segment(2) == 'whatsapp' && request()->segment(3) == 'reports']);
                }, ['icon' => '<svg aria-hidden="true" class="tw-size-5 tw-shrink-0" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" fill="none" stroke-linecap="round" stroke-linejoin="round">
                    <path stroke="none" d="M0 0h24v24H0z" fill="none"></path>
                    <path d="M3 21l1.65 -3.8a9 9 0 1 1 3.4 2.9l-5.05 .9"></path>
                    <path d="M9 10a.5 .5 0 0 0 1 0v-1a.5 .5 0 0 0 -1 0v1a5 5 0 0 0 5 5h1a.5 .5 0 0 0 0 -1h-1a.5 .5 0 0 0 0 1"></path>
            ...
            // ── Production Module (below WhatsApp) ─────────────────────
            $productionIcon = '<svg aria-hidden="true" class="tw-size-5 tw-shrink-0" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" fill="none" stroke-linecap="round" stroke-linejoin="round">
                    <path stroke="none" d="M0 0h24v24H0z" fill="none"/>
            ...
            if ($is_admin || auth()->user()->can('production.access') || $canProductionManager) {
                ...
                $menu->dropdown('Production', function ($sub) use ($is_admin, $workableStages, $stageLabels, $canProductionManager, $pmDashLabel) {
                    ...
                    if ($is_admin || auth()->user()->can('profit_loss_report.view')) {
                        $sub->url(route('reports.production-costs'), 'Production Costs', ['icon' => '', 'active' => request()->segment(2) == 'production-costs']);
                        $sub->url(route('reports.production-report'), 'Profit / Loss Report', ['icon' => '', 'active' => request()->segment(2) == 'production-report']);
                    }
                }, ['icon' => $productionIcon, 'active' => $productionActive])->order(17);
            ...
            // ── Raw Materials (below Production) ─────────────────────────
            if ($is_admin || auth()->user()->can('production.access') || \App\ProductionStageEmployee::where('user_id', auth()->id())->exists()) {
                $invIcon = '<svg aria-hidden="true" class="tw-size-5 tw-shrink-0" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" fill="none" stroke-linecap="round" stroke-linejoin="round">
                    <path stroke="none" d="M0 0h24v24H0z" fill="none"/>
            ...
            $todoActive = request()->segment(1) == 'employee-todos';
            ...
            if ($is_admin || auth()->user()->can('employee_todos.manage') || auth()->user()->can('send_notifications')) {
                ...
                }, ['icon' => $todoIcon, 'active' => $todoActive])->order(19);
            } elseif (auth()->user()->user_type === 'user' && ! auth()->user()->is_cmmsn_agnt) {
                $menu->url(route('employee-todos.my-week'), 'My To-Do', ['icon' => $todoIcon, 'active' => $todoActive])->order(19);
            }
                ...
                $menu->dropdown(
                    ...
                    function ($sub) use ($enabled_modules, $is_admin) {
                        if (auth()->user()->can('contacts_report.view')) {
                            $sub->url(
                                action([\App\Http\Controllers\ReportController::class, 'getCustomerSuppliers']) . '?contact_type=customer',
                                'ණය වාර්තාව',
                                ['icon' => '', 'active' => request()->segment(2) == 'customer-supplier' && request()->get('contact_type') == 'customer']
                            );
                        }
                        ...
                        if (auth()->user()->can('profit_loss_report.view') || auth()->user()->can('production.access') || auth()->user()->can('send_notifications')) {
                            $sub->url(
                                route('reports.production-costs'),
                                'Production Costs',
                                ['icon' => '', 'active' => request()->segment(2) == 'production-costs']
                            );
                            $sub->url(
                                route('reports.production-report'),
                                'Production Report',
            ...
            //Modules menu
            // Keep this block for future use. Toggle by .env MODULES_MENU_HIDDEN
            if (auth()->user()->can('manage_modules') && ! config('constants.modules_menu_hidden')) {
            ...
            if (auth()->user()->can('send_notifications')) {
                ...
                $menu->url(action([\App\Http\Controllers\SmsCampaignController::class, 'index']), __('lang_v1.sms_campaigns'), ['icon' => '<svg aria-hidden="true" class="tw-size-5 tw-shrink-0" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" fill="none" stroke-linecap="round" stroke-linejoin="round">
                    <path stroke="none" d="M0 0h24v24H0z" fill="none"></path>
                    <path d="M4 5a2 2 0 0 1 2 -2h12a2 2 0 0 1 2 2v10a2 2 0 0 1 -2 2h-7l-4 4v-4h-1a2 2 0 0 1 -2 -2z"></path>
                    <path d="M8 9h8"></path>
                    <path d="M8 13h5"></path>
                  </svg>', 'active' => request()->segment(1) == 'sms-campaigns'])->order(81);
            }
