...
class Util
{
    ...
    private function sendSmsViaTextlk($data)
    {
        $sms_settings = $data['sms_settings'];
        ...
        $numbers = array_filter(array_map('trim', explode(',', (string) $data['mobile_number'])));
        $response = null;

        foreach ($numbers as $number) {
            $recipient = $this->normalizeTextlkRecipient($number);
            if (empty($recipient)) {
                continue;
            }
            ...
            if (($body['status'] ?? '') === 'error') {
                throw new \Exception($body['message'] ?? 'TextLK SMS failed.');
            }
        }
    ...
    private function normalizeTextlkRecipient($number)
    {
        ...
        // Convert local Sri Lanka format 07XXXXXXXX -> 947XXXXXXXX
        if (strpos($number, '0') === 0 && strlen($number) === 10) {
            $number = '94'.substr($number, 1);
        }

        // Convert +94 / 94 formats with optional leading zeros to E.164 digits
    ...
    private function normalizeOutgoingSmsBody($body)
    {
        $body = (string) $body;

        $brandName = config('app.name', 'PrintWorks');
        $body = str_ireplace(['TEEN BEAUTY', 'Teen Beauty', 'Millennium agro', 'Millennium Agro'], $brandName, $body);
        $body = preg_replace('/\{business_name\}/i', $brandName, $body);

        // Standardize currency prefix in SMS content.
        $body = preg_replace('/\bRs\.?\s*/i', 'LKR ', $body);

        // Ensure transactional amount labels always carry LKR prefix.
        $body = preg_replace_callback(
            '/\b(total amount|due amount|paid amount|received amount)\b\s*[:\-]?\s*(?!LKR\b)([0-9][0-9,]*(?:\.\d{1,2})?)/i',
            function ($matches) {
                return $matches[1].': LKR '.$matches[2];
            },
            $body
        );
    ...
    {
        $sms_settings = $data['sms_settings'];
        $data['sms_body'] = $this->normalizeOutgoingSmsBody($data['sms_body'] ?? '');
        ...
        if ($sms_service == 'textlk') {
            return $this->sendSmsViaTextlk($data);
        }
    ...
    public function replaceTags($business_id, $data, $transaction, $contact = null)
    {
        ...
        foreach ($data as $key => $value) {
            ...
            if (strpos($value, '{sale_type}') !== false || strpos($value, '{payment_status}') !== false) {
                $due_for_type = $transaction->final_total - $total_paid;
                if ($due_for_type <= 0) {
                    $sale_type = 'Cash Sale';
                } elseif ($total_paid > 0) {
                    $sale_type = 'Partial Credit Sale';
                } else {
                    $sale_type = 'Credit Sale';
                }

                $payment_status = ucfirst((string) ($transaction->payment_status ?? 'due'));
                $data[$key] = str_replace('{sale_type}', $sale_type, $data[$key]);
                $data[$key] = str_replace('{payment_status}', $payment_status, $data[$key]);
            }
    ...
    public function createUser($request)
    {
        $user_details = $request->only([
            ...
            'cmmsn_percent', 'max_sales_discount_percent', 'dob', 'gender', 'marital_status', 'blood_group', 'contact_number', 'whatsapp_number', 'alt_number', 'family_number', 'fb_link',
        ...
        if (! empty($user_details['whatsapp_number'])) {
            $user_details['whatsapp_number'] = \App\Services\ProductionNotifier::normalizePhone($user_details['whatsapp_number']);
        }
        ...
        if ($user_details['user_type'] == 'user') {
            ...
            $is_admin = (strpos($role->name, 'Admin#') === 0);
            if (!$is_admin) {
