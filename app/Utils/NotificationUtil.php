...
class NotificationUtil extends Util
{
    ...
    public function autoSendNotification($business_id, $notification_type, $transaction, $contact)
    {
        ...
        if (! empty($notification_template)) {
            if ($notification_type == 'new_sale' || ! empty($notification_template->auto_send) || ! empty($notification_template->auto_send_sms) || ! empty($notification_template->auto_send_wa_notif)) {
                $orig_data = [
                ...
                if ($notification_type == 'new_sale') {
                    $contact_name = $contact->name ?? __('lang_v1.customer');
                    $invoice_number = $transaction->invoice_no ?? '';
                    $paid = ! empty($transaction->payment_lines) ? $transaction->payment_lines->where('is_return', '!=', 1)->sum('amount') : 0;
                    $due = (float) ($transaction->final_total ?? 0) - (float) $paid;
                    $total_amount = 'LKR '.number_format((float) ($transaction->final_total ?? 0), 2, '.', ',');

                    if ($due <= 0) {
                        $data['sms_body'] = "Dear {$contact_name}, Invoice No: {$invoice_number}. Sale Type: Cash Sale. Total Amount: {$total_amount}. Thank you for shopping with us.";
                    } else {
                        $formatted_due = 'LKR '.number_format((float) $due, 2, '.', ',');
                        $data['sms_body'] = "Dear {$contact_name}, Invoice No: {$invoice_number}. Sale Type: Credit Sale. Total Amount: {$total_amount}. Due Amount: {$formatted_due}.";
                    }
                }
