...
class ContactUtil extends Util
{
    ...
    /**
     * Create or restore the default walk-in customer for a business.
     */
    public function ensureWalkInCustomer($business_id, $user_id = null)
    {
        $walk_in = Contact::withTrashed()
        ...
        if ($walk_in) {
            ...
            $walk_in->is_default = 1;
            $walk_in->name = 'Walk-In Customer';
            $walk_in->first_name = 'Walk-In Customer';
            $walk_in->type = 'customer';
            $walk_in->contact_status = 'active';
            $walk_in->save();
        ...
        if (empty($user_id)) {
            $user_id = \App\Business::where('id', $business_id)->value('owner_id');
        }

        $businessUtil = new BusinessUtil();
        $ref_count = $businessUtil->setAndGetReferenceCount('contacts', $business_id);
        $contact_ref = $businessUtil->generateReferenceNumber('contacts', $ref_count, $business_id);
    ...
    /**
     * Check if contact is the default walk-in customer.
     */
    public function isWalkInCustomer($business_id, $contact_id): bool
    {
