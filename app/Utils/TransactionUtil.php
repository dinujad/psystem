...
class TransactionUtil extends Util
{
    ...
    public function getReceiptDetails($transaction_id, $location_id, $invoice_layout, $business_details, $location_details, $receipt_printer_type)
    {
        ...
        if ($il->show_letter_head == 1) {
            ...
            uploads_exists('invoice_logos/'.$il->letter_head) ?
            uploads_url('invoice_logos/'.$il->letter_head) : null;
        }

        //Logo
        $output['logo'] = $il->show_logo != 0 && ! empty($il->logo) && uploads_exists('invoice_logos/'.$il->logo) ? uploads_url('invoice_logos/'.$il->logo) : false;
        ...
        if ($transaction_type == 'sell' && $transaction->status == 'final') {
            ...
            foreach ($transaction->payment_lines as $payment_line) {
                if ($payment_line->method == 'bank_transfer' && $payment_line->is_return != 1) {
                    $bank_transfer_total += $payment_line->amount;
                }
            }
    ...
    public function getProfitLossDetails($business_id, $location_id, $start_date, $end_date, $user_id = null, $permitted_locations = null)
    {
        ...
        $productionCostService = app(\App\Services\ProductionCostService::class);
        $data['total_production_cost'] = $productionCostService->getTotalCostForProfitLoss($start_date, $end_date);
        ...
        if (! empty($data['total_production_cost']) && $data['total_production_cost'] > 0) {
            $data['left_side_module_data'][] = [
                'label' => 'Production Costs (completed jobs)',
                'value' => $data['total_production_cost'],
                'add_to_net_profit' => true,
            ];
            $module_total -= $data['total_production_cost'];
        }
    ...
    public function getPurchaseOrderPdf($business_id, $transaction_id)
    {
        ...
        //Logo
        $logo = $invoice_layout->show_logo != 0 && ! empty($invoice_layout->logo) && uploads_exists('invoice_logos/'.$invoice_layout->logo) ? uploads_url('invoice_logos/'.$invoice_layout->logo) : false;
