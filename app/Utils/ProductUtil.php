...
class ProductUtil extends Util
{
    ...
    public function decreaseProductQuantity($product_id, $variation_id, $location_id, $new_quantity, $old_quantity = 0)
    {
        ...
        if ($product->enable_stock == 1) {
            ...
            $previous_qty = (float) $details->qty_available;
            $details->decrement('qty_available', $qty_difference);
            $details->refresh();

            $this->sendLowStockWhatsappAlert($product, $details, $previous_qty);
        }

        return true;
    }

    private function sendLowStockWhatsappAlert($product, $variationLocationDetails, $previous_qty)
    {
        try {
            if (empty($product) || empty($variationLocationDetails)) {
                return;
            }

            $alert_qty = (float) ($product->alert_quantity ?? 0);
            if ($alert_qty <= 0) {
                return;
            }

            $current_qty = (float) ($variationLocationDetails->qty_available ?? 0);
            if ($current_qty > $alert_qty || $current_qty >= (float) $previous_qty) {
                return;
            }
            ...
            $admin_number = $sms_settings['admin_whatsapp_number'] ?? '';

            if (empty($admin_number)) {
                return;
            }

            $normalized_phone = $this->normalizeWhatsappNumber($admin_number);
            if (empty($normalized_phone)) {
                return;
            }

            $variation = Variation::find($variationLocationDetails->variation_id);
            $location = BusinessLocation::find($variationLocationDetails->location_id);
            $product_name = $product->name ?? 'Product';
            $variation_name = ! empty($variation) && strtoupper((string) $variation->name) !== 'DUMMY' ? ' - '.$variation->name : '';
            $location_name = $location->name ?? 'N/A';

            $message = "Low Stock Alert\n".
                "Business: ".config('app.name', 'PrintWorks')."\n".
                "Product: {$product_name}{$variation_name}\n".
                "Location: {$location_name}\n".
                "Alert Qty: {$alert_qty}\n".
                "Current Qty: {$current_qty}";

            $sender = app(\App\Services\LinkedWhatsappSender::class);
            if (! $sender->send($normalized_phone, $message)) {
                \Log::warning('Low stock WhatsApp alert failed — check linked WhatsApp at /whatsapp/link');
            }
        } catch (\Exception $e) {
            \Log::error('Low stock WhatsApp alert failed. File:'.$e->getFile().' Line:'.$e->getLine().' Message:'.$e->getMessage());
        }
    }

    private function normalizeWhatsappNumber($number)
    {
    ...
    public function getVariationStockHistory($business_id, $variation_id, $location_id)
    {
        $stock_history = Transaction::leftjoin('transaction_sell_lines as sl',
            'sl.transaction_id', '=', 'transactions.id')
                                ...
                                ->leftJoin('production_jobs as pj', 'pj.id', '=', 'transactions.production_job_id')
                                ...
                                ->select(
                                    ...
                                    'pj.customer_name as job_customer',
        ...
        foreach ($stock_history as $stock_line) {
            ...
            if ($stock_line->transaction_type == 'sell') {
                ...
                $quantity_change = $stock_line->purchase_line_quantity;
                $stock += $quantity_change;
                $isProductionJob = ! empty($stock_line->production_job_id);
                $stock_history_array[] = array_merge($temp_array, [
                    'quantity_change' => $quantity_change,
                    'stock' => $this->roundQuantity($stock),
                    'type' => 'production_purchase',
                    'type_label' => $isProductionJob ? 'Production Job' : __('manufacturing::lang.manufactured'),
                    'ref_no' => $stock_line->ref_no,
                    'additional_notes' => $stock_line->additional_notes,
                    'production_job_id' => $stock_line->production_job_id,
                    'job_number' => $stock_line->job_number,
                    'job_title' => $stock_line->job_title,
                    'job_customer' => $stock_line->job_customer,
                    'stock_in_second_unit' => $this->roundQuantity($stock_in_second_unit),
                ]);
