    ...
    public function getDataTables()
    {
        ...
        return DataTables::of($expenses)
            ...
            ->editColumn(
                'payment_status',
                <<<'BLADE'
<a href="{{ action('App\Http\Controllers\TransactionPaymentController@show', [$id]) }}" class="view_payment_modal payment-status" data-orig-value="{{$payment_status}}" data-status-name="{{__('lang_v1.' . $payment_status)}}"><span class="label @payment_status($payment_status)">{{__('lang_v1.' . $payment_status)}}</span></a>
BLADE
            )
